...
// Usage in server.js:  require('./privacy-routes')(app);
...
module.exports = function(app){
  ...
  const DSRRequest = mongoose.models.DSRRequest || mongoose.model('DSRRequest', new mongoose.Schema({
    ...
    name: String, email: String, country: String, details: String,
    idDoc: { filename: String, mime: String, size: Number, sha256: String, data: Buffer },
    ip: String, status: { type: String, enum: ['received','verifying','completed','rejected'], default: 'received' }
  }, { timestamps: true }));
  ...
  app.post('/api/privacy/dsr', upload.single('idDoc'), async (req, res) => {
    try {
      ...
      if (f && f.buffer) {
        ...
        idDoc = {
          filename: f.originalname, mime: f.mimetype, size: f.size,
          sha256: createHash('sha256').update(f.buffer).digest('hex'), data: f.buffer
        };
      }
      ...
      if (mailer && NOTIFY_TO) {
        ...
        const html = `
          <h3>New GDPR request</h3>
          <p><b>ID:</b> ${id}</p>
          <p><b>Type:</b> ${escape(type)}</p>
          <p><b>Name:</b> ${escape(name)} · <b>Email:</b> ${escape(email)}</p>
          <p><b>Country:</b> ${escape(country)}</p>
          <pre>${escape(details)}</pre>
        `;
        const attachments = idDoc ? [{ filename: idDoc.filename || 'id', content: idDoc.data, contentType: idDoc.mime }] : [];
        await mailer.sendMail({ from: NOTIFY_FROM, to: NOTIFY_TO, subject: `GDPR request: ${type} — ${email}`, html, attachments });
      }
