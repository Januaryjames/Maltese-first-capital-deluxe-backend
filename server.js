// server.js — Maltese First Capital API (Mongo + GridFS + JWT + Turnstile)
// Uses Node's built-in crypto.randomUUID (no 'uuid' package needed)
...
const { randomUUID } = require('crypto');              // <— built-in
...
// ---- Routes ----
app.get('/api/health', (_req,res)=> res.json({ ok:true, env:NODE_ENV, version:'1.4.2', uptime:process.uptime() }));
...
app.post('/api/auth/request-reset', async (req,res)=>{
  ...
  const token = randomUUID().replace(/-/g,'');           // <— was uuidv4()
...
app.post('/api/onboarding/submit',
  upload.fields([{name:'passport',maxCount:1},{name:'proofOfAddress',maxCount:1},{name:'companyDocs',maxCount:10},{name:'selfie',maxCount:1}]),
  requireTurnstile,
  async (req,res)=>{
    ...
    const applicationId = randomUUID();                  // <— was uuidv4()
...
app.get('/api/client/overview', authRequired('client'), async (_req,res)=>{
  res.json({ accounts:[ {id:'ACC-USD-001',currency:'USD',balance:5000000}, {id:'ACC-EUR-002',currency:'EUR',balance:1250000} ] });
});
